        ...
        for index, name in enumerate(keys):
            ...
            if content:
                ...
                result += create_structure(
                    content, indent + ("    " if last_item else "│  ")
                )
            else:
                result += f"{indent}{'└─ ' if last_item else '├─ '}{name}\n"
        return result
...
def repo(repo, selected_branch="main"):
    ...
    try:
        ...
        files = subprocess.check_output(
            ["git", "ls-tree", "--name-only", "-r", selected_branch]
        ).decode("utf-8")
        commits = (
            subprocess.check_output(
                ["git", "log", "--pretty=%B", "--first-parent", selected_branch]
            )
            .decode("utf-8")
            .strip()
        )
...
def file_viewer(repo, file, branch, nohighlight=""):
    ...
    try:
        ...
        last_commit = (
            subprocess.check_output(
                ["git", "log", "-1", branch, "--pretty=%B", "--", file_path]
            )
            .decode("utf-8")
            .strip()
        )
        ...
        file_size = (
            subprocess.check_output(["git", "cat-file", "-s", f"{branch}:{file_path}"])
            .decode("utf-8")
            .strip()
        )
    ...
    if (
        file_extension in plain_text_extensions
