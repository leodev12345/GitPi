...
def change_password():
    ...
    while True:
        ...
        result=bcrypt.checkpw(old_password.encode('utf-8'), config_dict["password"].encode('utf-8'))
        if result==True:
...
def change_storage():
    ...
    prompt=input(f"All of your repositories will be moved from {config_dict['storage_path']} to {new_location} do you want to continue[y/n]? ")
    if prompt=="Y" or prompt=="y":
        ...
        for entry in obj:
            if entry.is_dir() and ".git" in entry.name:
                ...
                shutil.move(old, new)
                print(f"Moved {entry.name} from {config_dict['storage_path']} to {new_location}")
        obj.close()
...
def import_repos():
    ...
    for entry in obj:
        ...
        if entry.is_dir() and ".git" in entry.name and entry.name.replace(".git","") not in data_dict:
            print(entry.name)
